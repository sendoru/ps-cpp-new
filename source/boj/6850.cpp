...
int main(void)
{
	...
	sort(dots.begin() + 1, dots.end(), [firstDot](vector2d a, vector2d b)
		{ ld ccwDiff = (b - firstDot).normSquare() * ccw(firstDot, vector2d(1, 0) + firstDot, a) * ccw(firstDot, vector2d(1, 0) + firstDot, a) * sgn(ccw(firstDot, vector2d(1, 0) + firstDot, a)) - (a - firstDot).normSquare() * ccw(firstDot, vector2d(1, 0) + firstDot, b) * ccw(firstDot, vector2d(1, 0) + firstDot, b) * sgn(ccw(firstDot, vector2d(1, 0) + firstDot, b));
	...
	area += convexHull.back().x * convexHull[0].y - convexHull.back().y * convexHull[0].x;
	cout << (ll)abs(area) / 100 << '\n';
