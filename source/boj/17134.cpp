...
int main() {
    ...
    auto is_prime = eratos(1e6 + 100);
    auto is_even_semiprime = vector<ll>(1e6 + 100, 0);
    for (int i = 0, j = 0; j < 1e6 + 100; i++, j += 2) {
        if (is_prime[i]) {
            is_even_semiprime[j] = 1;
        }
    }

    vector<ll> is_prime_conv(1e6 + 100, 0);
    for (int i = 0; i < 1e6 + 100; i++) {
        if (is_prime[i]) {
            is_prime_conv[i] = 1;
        }
    }
    convolve(is_prime_conv, is_even_semiprime);
    int t;
    cin >> t;
    while (t--) {
        int n;
        cin >> n;
        cout << is_prime_conv[n] << '\n';
    }
    return 0;
}