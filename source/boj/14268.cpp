...
int main(void) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (child != -1) {
            ...
            adjList[child].push_back(i);
        }
    }

    vector<int> levels(n + 1);
    vector<int> order;
    dfs(adjList, levels, order, 1, 1);
    ...
    lazy_segtree<ll, intSum, zero, ll, intSum, intSum, zero> tree(order.size());
    ...
    while (m--) {
        ...
        if (q1 == 1) {
            ll w;
            cin >> w;
            tree.apply(pos[q2][0], pos[q2][1] + 1, w);
        } else {
            cout << tree.prod(pos[q2][0], pos[q2][0] + 1) << '\n';
        }
    }
