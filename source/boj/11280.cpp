...
int main(void)
{
	...
	// A : 2 * A번에 할당
	// not A : 2 * A + 1번에 할당
	vector<set<int>> graph(2 * n + 2);
	vector<set<int>> reversedGraph(2 * n + 2);
	...
	for (int i = 0; i < m; i++)
	{
		...
		graph[notStart].insert(end);
		graph[notEnd].insert(start);
		reversedGraph[end].insert(notStart);
		reversedGraph[start].insert(notEnd);
	}
